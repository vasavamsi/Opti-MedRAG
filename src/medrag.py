...
class MedRAG:
    ...
    def medrag_answer(self, question, options=None, k=32, rrf_k=100, save_dir = None, snippets=None, snippets_ids=None, **kwargs):
        ...
        if self.rag:
            ...
            "Document [{:d}] (Title: {:s}) {:s}".format(
                idx, 
                retrieved_snippets[idx]["title"] if retrieved_snippets[idx]["title"] is not None else "", 
                retrieved_snippets[idx]["content"] if retrieved_snippets[idx]["content"] is not None else ""
            )
    ...
    def medrag_retrieve_snippets(self, question, k=32, rrf_k=100, save_dir = None, snippets=None, snippets_ids=None, **kwargs):
        '''
        question (str): question to be answered
        options (Dict[str, str]): options to be chosen from
        k (int): number of snippets to retrieve
