...
import tqdm
import numpy as np
import chardet  # Install this library with `pip install chardet` if not already installed.
...
class Retriever: 
    ...
    def get_relevant_documents(self, question, k=32, id_only=False, **kwarg):
        ...
        if id_only:
            ...
            # print('ind2txt', self.idx2txt(indices))
    ...
    #     '''
    #     for i in indices:
    #         print('file_paths :', os.path.join(self.chunk_dir, i["source"]+".jsonl"))
    ...
    #     for i in indices:
    #         file_path = os.path.join(self.chunk_dir, i["source"] + ".jsonl")
    ...
    #             print(f"UnicodeDecodeError encountered for file: {file_path}. Attempting conversion.")
    ...
    #                     try:
    #                         # Decode using detected encoding and re-encode as UTF-8
    ...
    #                         lines = []  # If decoding fails, set lines to empty
    #                 else:
    #                     print(f"Failed to detect encoding for file: {file_path}")
    #                     lines = []  # If encoding detection fails, set lines to empty
            
    #         # Extract the required line by index
    #         try:
    #             results.append(json.loads(lines[i["index"]]))
    #         except Exception as e:
    #             print(f"Error processing index {i['index']} in file {file_path}: {e}")
    #             results.append(None)  # Append None for invalid entries
    ...
    def idx2txt(self, indices): 
        ...
        for i in indices:
                ...
                with open(file_path, "rb") as f:
                    ...
                    if detected_encoding is not None:
                        ...
                        try:
                            ...
                            print(f"Failed to convert file: {file_path}. Error: {e}")
                            lines = []  # Set lines to empty
                    else:
                        print(f"Failed to detect encoding for file: {file_path}")
                        lines = []  # Set lines to empty

            # If all attempts fail, use a blank JSON with default keys
            if not lines:
                print(f"Using blank JSON for file: {file_path}")
                lines = ['{"id": null, "title": null, "content": null}']  # Substitute with default JSON
            ...
            try:
                ...
                print(f"Error processing index {i['index']} in file {file_path}: {e}")
                # Append a blank JSON object with default keys
                results.append({"id": None, "title": None, "content": None})
